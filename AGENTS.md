...
- **すべてのMDファイルは必ず `inbox/` ディレクトリに出力すること**（重要）

## コンテキスト収集フロー

### Step 1: 作業開始
1. **`inbox/` ディレクトリにMDファイルを保存**（唯一の出力先）
2. `run.bat` を実行（自動振り分け + Vector DB更新 + Git push）
3. YAMLフロントマターが自動付与され、適切なカテゴリへ移動

### Step 2: 分析プロセス
1. 既存コンテキストを検索・参照（RAG検索を使用）
2. 新しい洞察・分析結果を **`inbox/` に保存**（reports/等に直接保存しない）
3. `run.bat` を実行し、自動振り分けで適切なカテゴリへ整理

### Step 3: 精度向上
1. 思考ベクトルの策定: 戦略フレームワークを **`inbox/` に保存**
2. 情報参照: 過去のレポートからパターンを抽出（RAG検索）
3. レポート作成: 学習した内容を **`inbox/` に保存**（reports/に直接保存しない）
...
### ファイル出力に関する禁止事項
- **inbox/ 以外のディレクトリにMDファイルを直接保存しない**
  - ❌ `reports/` に直接保存
  - ❌ `strategy/` に直接保存
  - ❌ `analysis/` に直接保存
  - ❌ `memory/` に直接保存
...
### 手動処理に関する禁止事項
- 手動でYAMLフロントマターを編集しない（必要な場合はinbox/に保存し直す）
- 手動でカテゴリを変更しない（必要な場合はinbox/に保存し直す）
- Vector DBを手動で編集しない
